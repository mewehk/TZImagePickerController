...
/**
 视频预览中间正方形展示
 */
@property (nonatomic, assign) BOOL videoPreviewCenter;
...
@property (nonatomic, copy) void (^videoPreviewPageUIConfigBlock)(UIButton *playButton, UIView *toolBar, UIButton *doneButton, TZVideoPlayerController *viewController);
...
@property (nonatomic, copy) void (^videoPreviewPageDidLayoutSubviewsBlock)(UIButton *playButton, UIView *toolBar, UIButton *doneButton, AVPlayerLayer *playerLayer);
...
@property (nonatomic, copy) void (^photoCollectionDidTapItem)(TZAssetModel *model);

/**
  照片选择页---选择照片，视频不可点击时
 */
@property (nonatomic, copy) void (^photoPickerPageClickDisableBlock)(void);
#pragma mark - 图片预览页相关
