...
- (void)configBottomToolBar {
    ...
    if (_tzImagePickerVc.photoPreviewPageUIConfigBlock) {
        _tzImagePickerVc.photoPreviewPageUIConfigBlock(_collectionView, _naviBar, _backButton, _selectButton, _indexLabel, _toolBar, _originalPhotoButton, _originalPhotoLabel, _doneButton, _numberImageView, _numberLabel, self);
    }
}
...
- (void)select:(UIButton *)selectButton {
    ...
    if (!selectButton.isSelected) {
        ...
        if (_tzImagePickerVc.selectedModels.count >= _tzImagePickerVc.maxImagesCount) {
            NSString *title;
            if (_tzImagePickerVc.maxCountLimitStr) {
                title = [NSString stringWithFormat:_tzImagePickerVc.maxCountLimitStr, _tzImagePickerVc.maxImagesCount];
            } else {
            ...
            if (model.type == TZAssetModelMediaTypeVideo && !_tzImagePickerVc.allowPickingMultipleVideo) {
                [_tzImagePickerVc showAlertWithTitle: @"视频与图片不支持同时选择"];
                return;
            }
